...
export function setupNumberPortingRoutes(app: Express, sessionAuth: RequestHandler): void {
  app.get('/api/porting/countries', sessionAuth, async (_req: Request, res: Response) => {
    try {
  ...
  app.post('/api/porting/check', sessionAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.post('/api/porting/requests', sessionAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.get('/api/porting/requests', sessionAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      if (!userId) return res.status(401).json({ error: 'Unauthorized' });
  ...
  app.get('/api/porting/requests/:id', sessionAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      if (!userId) return res.status(401).json({ error: 'Unauthorized' });
  ...
  app.get('/api/porting/requests/:id/loa', sessionAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      if (!userId) return res.status(401).json({ error: 'Unauthorized' });
  ...
  app.patch('/api/porting/requests/:id', sessionAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.delete('/api/porting/requests/:id', sessionAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      if (!userId) return res.status(401).json({ error: 'Unauthorized' });
