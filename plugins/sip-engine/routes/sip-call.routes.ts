...
export function setupSipCallRoutes(
  app: Express,
  sessionAuth: RequestHandler
): void {
  app.get('/api/sip/calls', sessionAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.get('/api/sip/calls/:callId', sessionAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Not authenticated' });
  ...
  app.post('/api/sip/calls/initiate', sessionAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Not authenticated' });
