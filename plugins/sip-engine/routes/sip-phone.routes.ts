...
export function setupSipPhoneRoutes(
  app: Express,
  sessionAuth: RequestHandler
): void {
  app.get('/api/sip/phone-numbers', sessionAuth, async (req: Request, res: Response) => {
    try {
  ...
  app.post('/api/sip/phone-numbers', sessionAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Not authenticated' });
  ...
  app.post('/api/sip/phone-numbers/bulk-import', sessionAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Not authenticated' });
  ...
  app.patch('/api/sip/phone-numbers/:phoneNumberId', sessionAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Not authenticated' });
  ...
  app.delete('/api/sip/phone-numbers/:phoneNumberId', sessionAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Not authenticated' });
  ...
  app.post('/api/sip/phone-numbers/:phoneNumberId/assign-agent', sessionAuth, async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Not authenticated' });
