...
async function verifyWebhookSignature(req) {
  ...
  const [credential] = await db.select().from(elevenLabsCredentials).where(and(eq(elevenLabsCredentials.isActive, true), eq(elevenLabsCredentials.isPrimary, true))).limit(1);
