...
type PortRequest = typeof portRequests.$inferSelect;
type InsertPortRequest = typeof portRequests.$inferInsert;

const GCC_CARRIERS: Record<string, { name: string; carriers: string[] }> = {
  AE: {
...
const GCC_REGULATORS: Record<string, { body: string; note: string }> = {
...
export class NumberPortingService {
  ...
  static getCarriersForCountry(countryCode: string): string[] {
    return GCC_CARRIERS[countryCode]?.carriers || [];
  }

  static async createPortRequest(data: InsertPortRequest): Promise<PortRequest> {
  ...
  static async getUserPortRequests(userId: string): Promise<PortRequest[]> {
    return db
      .select()
      .from(portRequests)
      .where(eq(portRequests.userId, userId))
      .orderBy(desc(portRequests.createdAt));
  }

  static async getPortRequest(id: string, userId: string): Promise<PortRequest | null> {
    const [request] = await db
      .select()
      .from(portRequests)
      .where(and(eq(portRequests.id, id), eq(portRequests.userId, userId)))
      .limit(1);
    return request || null;
  }

  static async updatePortRequestStatus(
    id: string,
    status: string,
    adminNotes?: string
  ): Promise<PortRequest | null> {
  ...
  static async cancelPortRequest(id: string, userId: string): Promise<boolean> {
    const [request] = await db
      .select()
      .from(portRequests)
      .where(and(eq(portRequests.id, id), eq(portRequests.userId, userId)))
  ...
  static generateLoaText(data: Partial<InsertPortRequest>): string {
...
`.trim();
  ...
  static async checkPortability(phoneNumber: string, countryCode: string): Promise<{
    portable: boolean;
    message: string;
    ...
    const estimatedDays: Record<string, number> = {
      AE: 10, SA: 14, BH: 5, KW: 7, OM: 10, QA: 7,
    };
