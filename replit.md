...
-   **Real-Time Agent Assist System**: Per-agent configurable live call assistance that monitors compliance, tracks required call steps, detects sentiment shifts, and injects actionable suggestions into the AI agent's context. Configuration stored in `agentAssistConfig` JSONB field on agents table. Core engine in `server/services/agent-assist.service.ts`. Integrated into Bedrock-Polly audio bridge (`processUserTurn` → analyze → inject via `buildSystemPromptForStructured`). Events broadcast via LiveMonitoring WebSocket (`agent_assist` event type). UI configuration in `client/src/components/AgentAssistSettings.tsx` within agent editor. Features: compliance rules (required/banned phrases, disclosures), ordered required call steps, sensitivity levels (low/medium/high), priority-based intervention (compliance > resolution > experience), and dedup to avoid repeating prompts.
...
AWS deployment artifacts are deprecated and removed from this repository.
Do not use EC2/AWS deployment scripts from this project.