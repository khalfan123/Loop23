...
import { db } from "../../../db";
import { plivoCalls, plivoCredentials, plivoPhoneNumbers, users, creditTransactions, contacts, campaigns, agents, flows } from "@shared/schema";
...
export class PlivoCallService {
  ...
  static async handleCallStatus(
    ...
    if (['completed', 'busy', 'failed', 'no-answer', 'canceled'].includes(status) && call.userId) {
      try {
        ...
        if (agentInfo?.type === 'flow' && agentInfo.flowId) {
          try {
            const [flow] = await db
              .select()
              .from(flows)
              .where(eq(flows.id, agentInfo.flowId))
              .limit(1);
  ...
  static async getCallStatusFromPlivo(callId: string): Promise<{
    ...
    try {
      ...
      const hangupCause = plivoCallDetails.hangupCause || plivoCallDetails.hangup_cause || null;
