...
import { OPENAI_VOICES, MODEL_TIER_CONFIG } from '../types/openai-types';
...
export class OpenAIAgentFactory {
  ...
  static createAgentConfig(params: {
    ...
    const timeGreeting = this.getTimeGreeting(language, timeOfDay);
    const timeContext = `CURRENT TIME CONTEXT: It is currently ${timeOfDay} (${now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}). When greeting the caller, use the appropriate time-based greeting in ${languageName} (e.g. "${timeGreeting}").
...
You have a knowledge base with product catalog, pricing, and business information.
...
- Step 1: Identify what the caller is really asking (their underlying need, not just the surface question).
- Step 2: Search for the most relevant information using precise keywords.
- Step 3: If results are strong, formulate a confident, specific response. If results are weak or partial, search again before responding.
- Step 4: Present the answer naturally, leading with the benefit that matters most to THIS caller.
...
- Present pricing confidently as YOUR pricing. Say "That plan is X per month" — never hesitate or hedge.
...
- Never give uncertain language when you have definitive data. If the KB says the price is $99, say "It's $99" not "I think it might be around $99."
...
LANGUAGE DETECTION: You have automatic language detection enabled. Listen carefully to the language the caller is speaking and ALWAYS respond in the SAME language they use. If they switch languages, you should switch too. Support all major world languages naturally.`;
        ...
        case 'lookup_knowledge_base':
          handler = async (params: Record<string, unknown>) => {
            try {
              ...
              const toolResponse: Record<string, unknown> = { found: true, information: formattedResponse };
              if (extractedAnswer) toolResponse.directAnswer = extractedAnswer;
              return toolResponse;
            } catch (error: any) {
              console.error(`[KB Tool] Error:`, error.message);
  ...
  static async compileFlow(
    ...
    const hour = now.getHours();
    const timeOfDay = hour < 12 ? 'morning' : hour < 17 ? 'afternoon' : 'evening';
    const timeContext = `CURRENT TIME CONTEXT: It is currently ${timeOfDay} (${now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}). When greeting the caller, use the appropriate time-based greeting (e.g. "Good ${timeOfDay}").
...
GREETING RULES: Your opening greeting must ONLY include the time-based greeting, the company name (if known), your name (if known), and ask how you can help. NEVER mention any products, plans, prices, or offers in the greeting. Do NOT search the knowledge base until the caller states their needs.`;
  ...
  private static getTimeGreeting(langCode: string, timeOfDay: string): string {
    const greetings: Record<string, Record<string, string>> = {
      ...
      ur: { morning: 'صبح بخیر', afternoon: 'سلام', evening: 'شام بخیر' },
      fa: { morning: 'صبح بخیر', afternoon: 'عصر بخیر', evening: 'شب بخیر' },
    };
