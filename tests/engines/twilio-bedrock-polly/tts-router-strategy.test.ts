...
describe('buildTTSRouteContext cross-provider gating', () => {
  ...
  it('never offers deprecated cartesia, even with cross-provider on and a voice id', () => {
    // Cartesia is deprecated in production (provider removed); the route
    // context must refuse it regardless of policy or agent config.
    const ctx = buildTTSRouteContext(dualVoiceAgent, 'elevenlabs', 'hi there', on);
    expect(ctx.buildRequest('cartesia')).toBeNull();
    expect(ctx.buildRequest('elevenlabs')).not.toBeNull();
  });

  it('routes a cartesia-preferring agent to Polly (deprecation fallback)', () => {
    const ctx = buildTTSRouteContext(
      agent({ ttsProvider: 'cartesia', cartesiaVoiceId: 'ct-voice' }),
      'cartesia',
      'hi there',
      off
    );
